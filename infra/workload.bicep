...
resource app 'Microsoft.App/containerApps@2024-03-01' = {
  ...
  properties: {
    ...
    configuration: {
      ...
      registries: [ { server: '${registry.name}.azurecr.io'; username: credentials.username; passwordSecretRef: 'acr-password' } ]
      secrets: [
        { name: 'acr-password'; value: credentials.passwords[0].value }
        { name: 'openai-key'; value: openAiApiKey }
        { name: 'lab-token'; value: labToken }
      ]
    }
    template: {
      containers: [ {
        ...
        env: [
          ...
          { name: 'LAB_TOKEN'; secretRef: 'lab-token' }
          { name: 'LAB_VULNERABLE_MODE'; value: 'true' }
        ]
      } ]
